...
class Renamer{
  constructor(){
     ...
     .message-group .renamer-tag{ margin-right: 4px;}
     .message-group:not(.compact) .renamer-tag{margin-left:6px;}
	 .ui-color-picker-swatch{margin-bottom:10px; border: 1px solid black;}
	 .ui-color-picker-swatch.large{width: 100px; height: 50px;}
	 .ui-color-picker-swatch.selected{border: 2px solid black;}
		'</style>`;
		this.defaultSettings = { globals:{ "74822222203584512": {id: "74822222203584512", username: "ðŸŽ®MitchellðŸŽ®", discriminator: "5985", nick: "God King Emperor Mitchell", colour:"#FFD700", tag:{text: "Plugin Dev", back: "rgb(201, 63, 115)",fore: "#FFF"} },
                                       ...
                                       "134509976956829697": {id: "134509976956829697", username: "thecakeisalie25", discriminator: "0517", tag:{text: "Wonky Donky", back: "rgb(52, 152, 219)",fore: "#FFF"} },
                                       "164837968668917760": {id: "164837968668917760", username: "kdubious", discriminator: "0517", tag:{text: "The Great Fixer", back: "rgb(153, 45, 34)",fore: "#FFF"} } } }
		this.contextMarkup =
  ...
  setSwatches(currentColor,colorOptions,parent){
    ...
    .appendTo(parent);
	console.log("test");
    if (selection>-1) parent.find(".regulars .ui-color-picker-swatch").eq(selection).addClass(".selected")
  ...
  showInputModal(user){
    ...
    .on("click",".callout-backdrop,button.btn-default",(e)=>{modal.remove()})
	  .on("click",".ui-color-picker-swatch:not(.custom)",(e)=>{modal.find(".ui-color-picker-swatch.selected").removeClass("selected");e.target.classList.add("selected");custom.css("backgroundColor","")} )
	  custom = $(".ui-color-picker-swatch.custom").spectrum({
		  showInput:true,
		  showButtons: false,
		  move:(color)=>{$(".ui-color-picker-swatch.selected").removeClass("selected");custom.css("backgroundColor",color.toRgbString()).addClass("selected")}
	  })
